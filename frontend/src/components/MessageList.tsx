...
const VenueCard = ({ venue, onShowOnMap }: { venue: Venue; onShowOnMap: () => void }) => {
  ...
  console.log(`🖼️ VenueCard: ${venue.name}`, {
    hasPhotoUrl: !!venue.photoUrl,
    photoUrl: venue.photoUrl?.substring(0, 80) + '...'
  });
  ...
  return (
    <div className="bg-[#071620] border border-[#0f2a3a] rounded-lg overflow-hidden hover:shadow-md transition-shadow">
      {/* Image Section */}
      {venue.photoUrl && (
        <div className="w-full h-32 sm:h-40 overflow-hidden bg-gray-800">
          <img 
            src={venue.photoUrl} 
            alt={venue.name}
            className="w-full h-full object-cover"
            onError={(e) => {
              console.error(`❌ Failed to load image for ${venue.name}`);
              (e.target as HTMLImageElement).style.display = 'none';
            }}
            onLoad={() => {
              console.log(`✅ Image loaded for ${venue.name}`);
            }}
          />
        </div>
      )}
      ...
      <div className="p-3">
        <div className="flex justify-between items-start gap-2">
          <div className="flex-1">
            ...
            {/* Description - Option A: Not available from Text/Nearby Search */}
            {venue.description && (
              <p className="text-xs text-gray-300 mt-2 line-clamp-2">
                {venue.description}
              </p>
