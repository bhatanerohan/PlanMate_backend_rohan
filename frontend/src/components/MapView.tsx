...
const PopupContent = React.memo(({ marker }: { marker: MapMarker }) => {
  if (marker.type === 'venue') {
    const venue = marker.data as Venue;
    
    // 🔍 DEBUG: Log photo data in popup
    console.log(`🗺️ PopupContent: ${venue.name}`, {
      hasPhotoUrl: !!venue.photoUrl,
      photoUrlPreview: venue.photoUrl?.substring(0, 60)
    });
      ...
      <div className="min-w-[200px] sm:min-w-[250px] max-w-[300px]">
        ...
        {venue.photoUrl && (
          <img 
            ...
            className="w-full h-32 object-cover rounded-t"
            onError={(e) => {
              console.error(`❌ Popup image failed for ${venue.name}`);
              (e.target as HTMLImageElement).style.display = 'none';
            }}
            onLoad={() => {
              console.log(`✅ Popup image loaded for ${venue.name}`);
            }}
          />
        )}
        ...
        <div className="p-3">
          ...
          {/* Description - Option A: Not available */}
          {venue.description && (
            <p className="text-xs text-gray-700 mb-2 line-clamp-3">
              {venue.description}
            </p>
